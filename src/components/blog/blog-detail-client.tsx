...
import { bcp47Map } from "@/lib/currency"
import { Clock, ArrowLeft, ArrowRight } from "lucide-react"
import type { BlogPost } from "@/lib/blog"

function TableOfContents({ content, label }: { content: string; label: string }) {
  ...
  return (
    <nav className="p-5 mb-8 bg-muted/30" style={{ borderRadius: "var(--card-radius)", border: "var(--border-w) solid var(--border)" }}>
      <p className="font-mono text-[10px] tracking-widest uppercase text-primary/60 mb-3">{label}</p>
...
export function BlogDetailClient({ post, related }: BlogDetailClientProps) {
  ...
  return (
    <article className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          {publishDate && (
            <>
              ...
              <time dateTime={post.published_at || ""} className="text-foreground/40 text-xs">
                {publishDate.toLocaleDateString(bcp47Map[locale] || "en-US", { year: "numeric", month: "long", day: "numeric" })}
              </time>
            </>
          )}
        </div>
      ...
      {/* Table of Contents */}
      <TableOfContents content={post.content} label={t("blog.contents")} />
      ...
      <div className="mt-12 pt-8 border-t border-foreground/10">
        <div className="flex items-center gap-4">
          <span className="text-foreground/40 text-sm font-sans">{t("blog.share")}:</span>
          <a
          ...
          <button
            className="text-foreground/40 hover:text-primary transition-colors text-sm font-sans"
            onClick={() => navigator.clipboard.writeText(`https://goldenplanetwatches.com/blog/${post.slug}`)}
          >
            {t("blog.copyLink")}
          </button>
        </div>
      ...
      <div className="mt-8 p-6 bg-muted/30 text-center" style={{ borderRadius: "var(--card-radius)", border: "var(--border-w) solid var(--border)" }}>
        <p className="font-serif text-xl font-bold mb-2">{t("blog.questionsTitle")}</p>
        <p className="text-foreground/50 text-sm font-sans mb-4">{t("blog.questionsDesc")}</p>
        <a
      ...
      {related.length > 0 && (
        <div className="mt-16">
          <h2 className="font-serif text-2xl font-bold mb-6">{t("blog.relatedArticles")}</h2>
