...
**Data layer:** Supabase (PostgreSQL + RLS) is the source of truth for the live site. Watches are loaded from Supabase via `/api/watches` → `getWatches()` in `src/lib/db.ts` (no static fallback there). The detail page (`getWatchById` in `src/lib/get-watches.ts`) has a static fallback that only fires if Supabase errors or returns nothing — in production it does not run. Files in `src/data/` (`watches.ts`, `brands.ts`, `gp-stock.ts`) are seed/dev fixtures, not live data. Any user-visible content change (new watch, price update, sold-out flag, image swap) must land as a row change in Supabase — via SQL, the Supabase Table Editor, or the `PUT /api/watches` endpoint. Editing static files alone does nothing on the live site. Environment variables in `.env.local` (`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`).

**Watches schema:** Columns include `id`, `brand`, `model`, `reference`, `price`, `description`, `specs` (jsonb), `category`, `condition`, `gender`, `scope`, `featured`, `sold_out` (boolean, default false). Images live in a separate `watch_images` table joined by `watch_id`. When adding a new column to the `Watch` type: (1) `ALTER TABLE watches ADD COLUMN ...` on Supabase, (2) wire reads in both `toWatch` functions (`src/lib/get-watches.ts` and `src/lib/db.ts`), (3) wire writes in `createWatch`/`updateWatch` in `src/lib/db.ts` and the `/api/seed` route.
